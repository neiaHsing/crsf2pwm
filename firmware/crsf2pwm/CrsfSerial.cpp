...
void CrsfSerial::handleSerialIn()
{
    ...
    {
        //Serial.println("Csrf uart available.");
        uint8_t b = _port.read();
        _lastReceive = millis();

        if (getPassthroughMode())
        {
            Serial.println("Csrf uart passthroughmode.");
            if (onOobData)
                onOobData(b);
            continue;
        }
...
void CrsfSerial::packetChannelsPacked(const crsf_header_t *p)
{
    ...
    /*
    uint32_t t = millis();
    if(  t - _lastChannelsPacket > 500)
    {
      _lastChannelsPacket = t;
        if (onPacketChannels)
          onPacketChannels();
    }
    */
